...
- A **high-level, interpreted** programming language used to build interactive web pages/apps.
- Runs directly in the **browser**, and also on servers via **Node.js**.
- **Dynamically typed** → variable types are decided at runtime (you don't declare types).

```js
let x = 5; // number
x = "hello"; // now a string — JS allows this, no error
```
...
- A **superset of JavaScript** — every valid JS code is valid TS code.
- Adds **static typing** (you declare what type a variable should hold).
- TS code doesn't run directly in browsers — it's **transpiled to JS** using the compiler `tsc`.
...
**Why it matters:** TS catches this mistake _before_ you run the code. JS would only fail (or silently misbehave) at runtime.
...
- `outDir` – folder where compiled JavaScript files will be generated.

### Compile a TypeScript File

```bash
npx tsc filename.ts     # → creates filename.js
```
...
Instead of compiling first, use **tsx**:
...
Same syntax in both JavaScript and TypeScript.
...
1. `var` — outdated, has scoping issues
2. Automatically (no keyword) — bad practice, creates a global variable
...
```js
let userName = "Alex";   // ✅ valid
let $price = 100;        // ✅ valid
let _count = 0;          // ✅ valid
let 1name = "test";      // ❌ invalid — starts with a digit
let let = 5;              // ❌ invalid — reserved word
```

Same rules apply identically in TypeScript.
...
A **variable** is a named container that stores data in memory so it can be used later.
...
```typescript
let car: string = "Volvo";
```

The only difference is that **TypeScript allows specifying the data type**.
...
| Keyword | Recommended | Mutable | Scope    |
| ------- | ----------- | ------- | -------- |
| const   | ⭐ Yes      | ❌      | Block    |
| let     | ✅ Yes      | ✅      | Block    |
| var     | ❌ No       | ✅      | Function |
...
Use **const** whenever the variable **should never be reassigned**.
...
It only protects the **reference**, not the contents.
...
Avoid using it in modern applications.
...
Before execution, JavaScript runs a **memory creation phase** (also called the compilation phase). During this phase, the JS engine scans the current scope and sets up memory for every declaration **before** running any code line by line.

This is why it _looks like_ declarations "move" to the top of their scope.

Only the **declaration** moves — not the **initialization/assignment**. What differs between `var`, `let`, `const`, functions, and classes is **what value the engine gives them during that memory phase**, and **whether you're allowed to touch them before their line runs**.

```
Phase 1: Memory Creation (Hoisting)
  → var x           → x = undefined
  → let y, const z  → y, z = "uninitialized" (in TDZ)
  → function foo(){}→ foo = actual function (fully hoisted)

Phase 2: Execution
  → code runs top to bottom, line by line
  → values get assigned as their lines execute
```
...
`var` is hoisted **and initialized to `undefined`** during the memory phase, so accessing it early doesn't throw — it just gives you `undefined`.
...
```javascript
var name; // hoisted + auto-initialized to undefined

console.log(name);

name = "John"; // assignment happens only when this line runs
```

### var inside functions and loops

```javascript
function greet() {
  console.log(msg); // undefined, not an error
  var msg = "Hi";
}
...
console.log(i); // 3 — "i" leaks outside the loop because var is function/global scoped
```
...
Regular function declarations are hoisted **completely** — both the name and the body — so you can call them before their line in the code.
...
Here, only `var sayBye` is hoisted (as `undefined`). The function value is assigned only when execution reaches that line — so calling it early tries to call `undefined()`.

Arrow functions behave the same way when assigned to `var`/`let`/`const`.

---

## let Hoisting

`let` **is** hoisted (the engine reserves memory for it in the memory phase), but it is **not initialized**. It stays in the **Temporal Dead Zone (TDZ)** until its declaration line actually executes.
...
Note the error message itself — it confirms the engine already _knows_ `name` exists (that's the hoisting part); it just isn't usable yet (that's the TDZ part).

---

## const Hoisting

Same as `let` — hoisted, but stuck in the TDZ until initialized. `const` additionally _requires_ that initialization happen on the same line it's declared.
...
Classes are hoisted too, but like `let`/`const`, they land in the TDZ.
...
This surprises people because function declarations _can_ be used before they're defined, but classes cannot — even though both look like "define once, use anywhere" constructs.

---

## Temporal Dead Zone (TDZ) — In Detail

The **TDZ** is the span of code between:

1. the start of the variable's scope (where it gets hoisted), and
2. the line where it is actually declared/initialized.

During that span, the variable technically exists in memory, but touching it in _any_ way throws a `ReferenceError`.
...
Start of Scope
       ...
       ▼
┌───────────────────────────┐
│   TEMPORAL DEAD ZONE       │   <- variable exists but is LOCKED
│   (accessing it throws)    │
└───────────────────────────┘
       │
       ▼
   let age = 20;      <- TDZ ends here, variable is now usable
       │
       ▼
   Rest of the scope (age is fully accessible)
```

### Even `typeof` is not safe in the TDZ

Normally `typeof undeclaredVar` returns `"undefined"` safely. But inside the TDZ, even `typeof` throws:
...
Compare with a variable that was never declared at all:

```javascript
console.log(typeof neverDeclared); // "undefined" — no error
```

This is a common interview trap: "does `typeof` always fail silently?" — **no**, not for `let`/`const` variables still in their TDZ.

### TDZ is scope-specific, not just "before the line"

The TDZ is tied to the **block**, so shadowing can create confusing traps:
...
Even though an outer `value` exists, the inner `let value` shadows it for the _entire_ function block — including lines above its own declaration — so the inner variable is in its TDZ and blocks access to the outer one too.
...
Default parameter values are evaluated left to right, and `b` is still in its TDZ when `a`'s default tries to use it.

---

## Why TDZ Exists

It's not a design accident — it's meant to catch real bugs:

- It forces you to declare variables before using them, preventing silent `undefined` bugs that `var` allows.
- It makes `const` reliable — the engine can guarantee no code path reads a `const` before it has its real value.
- It surfaces temporal ordering mistakes (like the shadowing example above) as loud errors instead of quiet wrong values.

---

## Hoisting Comparison Table

| Declaration Type       | Hoisted?               | Initial Value in Memory Phase | Usable Before Declaration Line? |
| ---------------------- | ---------------------- | ----------------------------- | ------------------------------- |
...
| `function` declaration | ✅ Yes                 | Full function                 | ✅ Yes — fully callable         |
| `function` expression  | ✅ Yes (var part only) | `undefined`                   | ❌ No — `TypeError`             |
...
TypeScript's type system gives you two complementary ways to make sure a variable, parameter, or return value has the type you expect: **type annotation** (you tell TypeScript the type) and **type inference** (TypeScript figures out the type on its own). Understanding when each one kicks in — and when to lean on one over the other — is one of the fastest ways to write cleaner, safer TypeScript.

---

## Type Annotation

With type annotation, you **explicitly specify the type** using a colon (`:`) followed by the type.
...
Annotations are especially useful when:

- The variable is declared without an initial value.
- The type can't be inferred correctly on its own (e.g. `any`-prone situations).
- You want to enforce a specific shape via an `interface` or `type`.
...
TypeScript **automatically determines the type** based on the assigned value, without you writing it explicitly.

```typescript
let age = 25;
```

TypeScript infers that `age` is of type `number`. If you later try:

```typescript
age = "twenty-five"; // ❌ Error: Type 'string' is not assignable to type 'number'.
```

TypeScript will throw a compile-time error, because it locked in the type the moment `age` was initialized.

Inference also works with:

```typescript
let user = { id: 1, name: "Asha" };
...
| Aspect               | Type Annotation                                             | Type Inference                          |
| -------------------- | ----------------------------------------------------------- | --------------------------------------- |
| Who defines the type | Developer, explicitly                                       | TypeScript compiler                     |
| Syntax               | `let x: number = 5;`                                        | `let x = 5;`                            |
| Readability          | Very clear, self-documenting                                | Slightly less explicit                  |
| Verbosity            | More code to write                                          | Less code, cleaner                      |
| Best for             | Function signatures, uninitialized variables, complex types | Local variables with obvious values     |
| Error catching       | Same — both are checked at compile time                     | Same — both are checked at compile time |

---

## When to Use Type Annotation

- **Function parameters** — TypeScript can't infer these from context, so annotate them:
  ```typescript
  function multiply(a: number, b: number): number {
    return a * b;
  }
  ```
- **Variables declared without a value:**
  ```typescript
  let result: number;
  result = compute();
  ```
- **Public APIs / library code** — explicit types make the contract clear to anyone consuming your code.
- **Complex or union types** where inference might be ambiguous:
  ```typescript
...
- **Local variables initialized immediately:**
  ```typescript
  let total = price * quantity;
  ```
- **Simple return values** where the logic makes the type obvious.
- **Reducing visual noise** in straightforward code — letting TypeScript do the work keeps things concise without losing safety.

---

## Common Pitfalls

1. **Relying on inference for `let` without immediate assignment:**

   ```typescript
   let value; // inferred as `any`
   value = 10;
   value = "now a string"; // no error — inference didn't help here
   ```

   Fix: annotate it — `let value: number;`

2. **Over-annotating obvious values:**

   ```typescript
   let count: number = 5; // redundant, inference already gives you `number`
   ```

   Prefer `let count = 5;` for simple cases.

3. **Wide inference on empty arrays/objects:**
   ```typescript
   let items = []; // inferred as any[]
   ```
   Fix: annotate — `let items: string[] = [];`

---

## Best Practices

- Let TypeScript infer types for simple, immediately-initialized local variables.
- Explicitly annotate function parameters and return types, especially in shared or public code.
- Annotate variables that are declared before being assigned a value.
- Use interfaces or type aliases for complex object shapes instead of relying on inline inference.
- Don't annotate when the type is already obvious — it just adds noise.

---

# What is the `typeof` Operator?

`typeof` tells you the **data type** of a value. It returns the type as a **string**.

- **JavaScript:** works at runtime only.
- **TypeScript:** works at runtime (same as JS) + at compile time to create types.
...
To properly check arrays, use `Array.isArray(value)` instead of `typeof`.
...
## TypeScript: Creating Types with `typeof`

TypeScript can reuse a variable's shape as a type, so you don't write it twice.
...
TypeScript uses **static typing** — it checks types **before** the code runs. There are two ways to assign types:

1. **Explicit Typing** – you specify the type manually.
2. **Type Inference** – TypeScript detects the type automatically.
...
**Use for:** function parameters/return types, object shapes, and any variable whose type isn't obvious.
...
```ts
let username = "John"; // inferred: string
...
**Use for:** simple variables that are initialized immediately and have an obvious type.

---

## Type Safety

Both approaches catch mismatches at **compile time**, before the code runs:
...
```js
// JavaScript — silently concatenates strings
function add(a, b) {
  return a + b;
}
add("5", 3); // "53"
...
TypeScript uses `any` when it can't infer a type — this disables type checking:

```ts
const data = JSON.parse('{"name":"Alice"}'); // type: any

let value; // type: any
value = "Hello";
value = 100; // no error, because any = no checking
```

**Avoid `any`** — it removes IntelliSense, hides bugs, and makes debugging harder.

**Better alternatives:**

- Use explicit annotations: `let age: number = 25;`
- Use interfaces: `interface User { name: string; age: number }`
- Use type guards: `if (typeof value === "string") { ... }`
- Enable `"noImplicitAny": true` in `tsconfig.json` to catch accidental `any` usage.
...
| --------------------------------- | ---------------------------------- |
| Type is manually specified        | Type is auto-detected              |
| Best for functions, objects, APIs | Best for simple, obvious variables |
...
- Use **type inference** for simple, obvious variables — less code, same safety.
- TypeScript catches type errors at **compile time**, preventing runtime bugs.
- Avoid `any`; enable `noImplicitAny` for stronger type safety.
