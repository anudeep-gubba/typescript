...
❌ Compile-time Error because the constructor expects a `string`.
...
- **Getter** → Returns the value of a property (**Accessor**).
- **Setter** → Updates the value of a property after performing validation (**Mutator**).
...
- **Parent (Base) Class** → Class being inherited.
- **Child (Derived) Class** → Class that inherits from the parent.
...
| Must Be Extended          | ❌ No         | ✅ Yes (to use it) |
...
| Supported in JavaScript  | ✅ Yes                | ❌ No                     |
